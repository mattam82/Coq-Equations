...
let pr_context env sigma c =
  let pr_decl env decl =
    ...
    let idstr = match Context.binder_name id with Name id -> Id.print id | Anonymous -> str"_" in
...
let lets_of_ctx env ctx evars s =
  ...
  let ctxs, pats, varsubst, len, ids = 
    fold_left (fun (ctx', cs, varsubst, k, ids) (id, pat) -> 
        ...
        | _ -> 
          ...
          (make_def (Context.make_annot (Name id) Sorts.Relevant)
  ...
  let _, _, ctx' = List.fold_right (fun decl (ids, i, ctx') ->
      let (n, b, t) = to_tuple decl in
      try ids, pred i, (make_def (Context.map_annot (fun _ -> Name (List.assoc i varsubst)) n) b t :: ctx')
      with Not_found -> 
    ...
    | None ->
      errorlabstrm "deppat"
        (str "Non-exhaustive pattern-matching, no clause found for:" ++ fnl () ++
         pr_problem data env !evars prob)
...
and interp_clause env evars data prev clauses' path (ctx,pats,ctx' as prob) lets ty
  ...
  | By (tac, s) ->
    ...
    if Proofview.finished res then
      ...
      let solve_goal gl =
        ...
        in 
        match covering_aux env evars data [] (List.map (fun x -> x, false) s) path prob lets ty' with
        | None ->
  ...
  | Refine (c, cls) -> 
    ...
    let decl = make_assum (make_annot (Name idref) Sorts.Relevant) (mapping_constr !evars revctx cty) in
