...
let curry_concl env sigma na dom codom =
  ...
  let terms, acc =
    ...
    | [] -> ([mkRel 1], mkRel 1)
  in
  let sigma, ev = Equations_common.new_evar env sigma newconcl in
...
module Tactics =struct
  ...
  enter { enter = fun gl ->
    ...
    let sigma', sigsig, sigpack =
      get_signature env (Sigma.to_evar_map sigma)
  ...
  enter { enter = fun gl ->
    ...
    pattern_sigma ~assoc_right:true term id env
