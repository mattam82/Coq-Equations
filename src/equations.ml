...
let inline_helpers i = 
  let l = List.map (fun (_, _, id) -> Libnames.Ident (dummy_loc, Obj.magic id)) i.helpers_info in
...
let define_by_eqs opts eqs nt =
  ...
  let interp_arities (((loc,i),rec_annot,l,t),_ as ieqs) =
	...
	let compapp = mkApp (compc, rel_vect 0 (length sign)) in
        ...
        Table.extraction_inline true [Libnames.Ident (dummy_loc, Obj.magic compid)];
    ...
    | Some b ->
	 ...
	 Declare.declare_constant projid
       ...
       Table.extraction_inline true [Libnames.Ident (dummy_loc, Obj.magic projid)];
