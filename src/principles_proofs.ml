...
let aux_ind_fun info chop nested unfp unfids p =
  ...
  and aux_program lctx chop unfp unfids porig p =
    ...
    | None ->
      ...
      in
      tclTHEN (to82 fixtac)
        (aux chop unfs unfids p.program_splitting)
    | Some t ->
      ...
      tclTHENLIST [to82 intros;
                   tclDO t.rec_args (to82 revert_last);
                   observe "wf_fix"
                     (tclTHEN refine
            ...
            let wheretac =
              ...
              observe_new "one where"
                (tclTHENLIST [
                    ...
                    (of82 (aux_program lctx chop (Option.map (fun s -> s.where_program) unfs)
  ...
  | Some (Guarded l) :: context ->
     ...
     let prove_progs progs =
       ...
       tclDISPATCH (List.map (fun (p,_unfp,cpi,e) ->
                    ...
                    in
                    (of82 (aux_ind_fun proginfo (0, List.length l) nestedinfo None []
    ...
    let tac =
      Proofview.tclBIND
        (tclCOMPLETE (tclTHENLIST
                        [set_eos_tac (); intros;
                         of82 (aux_ind_fun info (0, 0) nestedinfo unfp [] p)]))
  ...
  and aux subst base unfold_base split unfold_split =
    ...
    | Compute (_, wheres, _, RProgram _), Compute ((lctx, _, _), unfwheres, _, RProgram c) ->
       ...
       to82 (observe_new "compute"
         (Tacticals.New.tclTHENLIST [intros; of82 wheretacs;
                       observe_new "compute rhs" (Tacticals.New.tclTRY (of82 (unfolds base unfold_base)));
