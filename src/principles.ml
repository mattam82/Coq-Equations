  ...
  let newarity =
    if lenrealinds == 1 then
      ...
      let clean_one a sign fn =
        let ctx, concl = decompose_prod_assum a in
      ...
      let rec aux arity ind_stmts =
        match kind_of_term arity, ind_stmts with
...
let subst_rec_split env evd f comp comprecarg prob s split =
  ...
  let subst_rec_named s acc = 
    List.fold_left (fun (acc, ctx) (id, b) ->
        try let (_, _, ty) = to_named_tuple (lookup_named id ctx) in
...
let declare_funind info alias env evd is_rec protos progs
  ...
  try ignore(Obligations.add_definition
             ...
             indid (Reductionops.nf_evar !evd statement)
             ~tactic:(ind_fun_tac is_rec (Reductionops.nf_evar !evd f) info id split unfsplit progs) ctx [||])
  with e ->
...
let build_equations with_ind env evd ?(alias:(constr * Names.Id.t * splitting) option) progs =
  ...
  let declare_ind () =
    ...
    let ind =
      if poly then
        mkIndU ((kn,0),Univ.UContext.instance (snd (Evd.universe_context !evd)))
      else mkInd (kn,0)
    in
    let _ =
