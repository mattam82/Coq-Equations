...
let build_app_infer (env : Environ.env) (evd : Evd.evar_map ref) ((ctx, ty) : goal)
  (ctx' : Context.Rel.t) (f : Globnames.global_reference)
  ...
  let tB', body =
    ...
    if nondep then
      ...
      let body' = Term.it_mkProd_or_LetIn body after' in
        ...
        Constr.mkLambda (name', Vars.lift (-rel') tA', body'), body
    else
...
let maybe_pack : simplification_fun =
  ...
  if CList.is_empty args then
    ...
    (* FIXME Inserted this to simplify tB when it has the shape:
             ...
             Is this correct? *)
    let tB = let env = Environ.push_rel_context ctx env in
...
let elim_true : simplification_fun =
  ...
  if Vars.noccurn 1 ty2 then
    ...
    build_term env evd (ctx, ty) (ctx, Termops.pop ty2)
      (fun c -> Constr.mkLambda (name, ty1, Vars.lift 1 c)), subst
  else
...
let infer_step ~(loc:Loc.t) ~(isSol:bool)
  ...
  if check_constant (Lazy.force EqRefs.opaque_ind_pack_eq_inv) f then
    ...
    if isSol then
      if Term.isRel tu && Term.isRel tv then
        Solution (choose (Term.destRel tu) (Term.destRel tv))
      ...
      in
      if Term.isRel tu && Term.isRel tv && check_occur tu tv then
        Solution (choose (Term.destRel tu) (Term.destRel tv))
  ...
  let handle_error f =
    fun env evd gl ->
      ...
      with CannotSimplify err ->
        CErrors.user_err_loc (loc, "Equations.Simplify", err)
  in
  let wrap get_step =
