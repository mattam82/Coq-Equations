...
Equations(nocomp noind) Below_fin (P : Π n, fin n -> Type) {n} (v : fin n) : Type :=
...
Equations(nocomp noeqns noind) below_fin (P : Π n, fin n -> Type)
  (step : Π n (v : fin n), Below_fin P v -> P n v)
...
Definition rec_fin (P : Π n, fin n -> Type) {n} v
  (step : Π n (v : fin n), Below_fin P v -> P n v) : P n v :=
  step n v (below_fin P step v).

Instance fin_Recursor n : Recursor (fin n) :=
  { rec_type := λ v, Π (P : Π n, fin n -> Type) step, P n v;
