...
Section Relation_Definition.
  ...
  Section General_Properties_of_Relations.
    ...
    Definition reflexive : Type := forall x:A, R x x.
    Definition transitive : Type := forall x y z:A, R x y -> R y z -> R x z.
    Definition symmetric : Type := forall x y:A, R x y -> R y x.
    Definition antisymmetric : Type := forall x y:A, R x y -> R y x -> x = y.
  ...
  Section Sets_of_Relations.
    ...
    Record equivalence : Type :=
      { equiv_refl : reflexive;
	equiv_trans : transitive;
	equiv_sym : symmetric}.

    Record PER : Type :=  {per_sym : symmetric; per_trans : transitive}.
  ...
  Section Relations_of_Relations.
    ...
    Definition inclusion (R1 R2:relation) : Type :=
      forall x y:A, R1 x y -> R2 x y.

    Definition same_relation (R1 R2:relation) : Type :=
      inclusion R1 R2 /\ inclusion R2 R1.

    Definition commut (R1 R2:relation) : Type :=
      forall x y:A,
