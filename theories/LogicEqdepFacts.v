...
Module PathNotations.
  Notation "'rew' H 'in' H'" := (paths_rect _ _ (fun _ _ => _) H' _ H)
    (at level 10, H' at level 10,
     format "'[' 'rew'  H  in  '/' H' ']'").
  Notation "'rew' [ P ] H 'in' H'" := (paths_rect _ _ (fun x _ => P x) H' _ H)
    (at level 10, H' at level 10,
     format "'[' 'rew'  [ P ]  '/    ' H  in  '/' H' ']'").
(*
...
Section Dependent_Equality.
  ...
  Inductive eq_dep (p:U) (x:P p) : forall q:U, P q -> Type :=
  ...
  Proof.
    destruct 1.
    apply eq_dep1_intro with (idpath p).
    simpl; trivial.
  Qed.
...
Proof.
  intros U P p q x y H.
  refine (@transport _ (fun s => eq_dep p x (proj1_sig s) (proj2_sig s)) (p;x) (q;y) H _).
  apply eq_dep_intro.
Qed.
...
Proof.
  ...
  - change x2 with (projT1 (existT P x2 H2)).
    ...
    reflexivity.
  - destruct H.
    apply path_sigma with (p := proj1_sig).
    apply proj2_sig.
Defined.
...
Section Equivalences.
  ...
  Definition UIP_refl_on_ (x : U) :=
    forall (p : x = x), p = idpath x.
  Definition UIP_refl_ := forall x, UIP_refl_on_ x.

  (** Streicher's axiom K *)

  Definition Streicher_K_on_ (x : U) (P : x = x -> Type) :=
    P (idpath x) -> forall p : x = x, P p.
  ...
  Lemma eq_dep_eq_on__UIP_on (x y : U) (p1 : x = y) :
    Eq_dep_eq_on (fun y => x = y) x idpath -> UIP_on_ x y p1.
  Proof.
    intro eq_dep_eq; red.
...
Proof.
  ...
  { intros. apply @concat with (idpath x). apply UIP_refl.
    symmetry. apply UIP_refl. }
  transitivity (concat (concat (UIP (idpath x) (idpath x)) z)
                         (inverse (UIP (idpath x) (idpath x)))).
  - destruct z. destruct (UIP _ _). reflexivity.
  - change
      (match idpath x as y' in _ = x' return y' = y' -> Type with
       | eq_refl => fun z => z = (idpath (idpath x))
       end (concat (concat (UIP (idpath x) (idpath x)) z)
                     (inverse (UIP (idpath x) (idpath x))))).
    destruct z. destruct (UIP _ _). reflexivity.
Qed.
...
Lemma eq_rec_eq :
  forall (p:U) (Q:U -> Set) (x:Q p) (h:p = p), x = @paths_rec _ p (fun a => Q a) x p h.
...
Lemma UIP_refl : forall (x:U) (p:x = x), p = idpath x.
...
Lemma Streicher_K :
  forall (x:U) (P:x = x -> Type), P (idpath x) -> forall p:x = x, P p.
