...
Hint Extern 10 => is_ground_goal ; progress (elimtype Empty) : elimtype Empty.
...
Lemma JMeq_eq_refl {A} (x : A) : JMeq_eq (@JMeq_refl _ x) = idpath.
Proof. apply UIP. Qed.
...
Proof. intros H x E. rewrite E; assumption. Defined.
...
Proof.
intros F E.
unfold conditional_eq in F.
assert (H : p = q).
refine (@transport _ (fun s => p = (proj1_sig s)) (p;x) (q;y) E (idpath _)).
apply F; trivial.
Defined.
...
Ltac simplify_one_dep_elim_term c :=
  match c with
    ...
    | paths (existT _ _ _) (existT _ _ _) -> _ =>
    ...
    | ?t = ?u -> _ => let hyp := fresh in
      intros hyp ; elimtype Empty ; discriminate
