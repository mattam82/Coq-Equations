...
Notation " x = y " := (equality x y) : type_scope.

Equations(nocomp noind) eq_sym (A : Type) (x y : A) (p : equality x y) : equality y x :=
eq_sym A x ?(x) eq_refl := eq_refl.
  
Equations(nocomp) eq_trans {A : Type} {x y z : A}
         (p : equality x y) (q : equality y z) : equality x z :=
eq_trans A x ?(x) z eq_refl p := p.
Transparent eq_trans.

Lemma eq_trans_equation_2 {A : Type} {x y z : A} (p : equality x y) :
  eq_trans p eq_refl = p.
Proof. now destruct p. Defined.

Hint Rewrite @eq_trans_equation_2 : eq_trans.
...
Record sigma {A : Type} (P : A -> Type) := dpair
  { proj1 : A ; proj2 : P proj1 }.

Notation " { x : T & P } " := (sigma (fun x : T => P)).

Notation " ( x ; p ) " := (@dpair _ _ x p).
...
Equations(noind) ap {A B:Type} (f:A -> B) {x y:A} (p:x = y) : f x = f y :=
ap A B f x ?(x) eq_refl := eq_refl.
...
Equations(nocomp) apD10 {A} {B:A->Type} {f g : forall x, B x} (h:f=g) (x : A) : f x = g x :=
apD10 A B f g eq_refl x := eq_refl.
...
Equations(nocomp) transport {A : Type} (P : A -> Type) {x y : A} (p : x = y) (u : P x) : P y :=
transport A P x ?(x) eq_refl u := u.

Notation "p # x" := (transport _ p x) (right associativity, at level 65, only parsing).
Transparent transport.

Equations(nocomp) path_sigma {A : Type} (P : A -> Type) (u v : sigma P)
          (p : u.1 = v.1) (q : p # u.2 = v.2) : u = v :=
  path_sigma A P (dpair x y) (dpair ?(x) ?(y)) eq_refl p := _.

             Next Obligation. destruct p.  reflexivity. Defined.
