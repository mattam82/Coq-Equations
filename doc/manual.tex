...
\def\figdefs{\begin{array}{llcl}
  ...
  \texttt{notation} & not & \Coloneqq & \texttt{''}string\texttt{''}~\textcoloneq~term~(\texttt{:}~scope)?\\
  ...
  \texttt{clauses} & clauses & \Coloneqq & \vec{c} `| \texttt{\{}\,\vec{c}\,\texttt{\}} \\
  \texttt{user clause} & c & \Coloneqq & \coqdoccst{f}~\vec{up}~n `|
                                         \vecplus{\texttt{|}~up}~n\\
  \texttt{user pattern} & up & \Coloneqq & x
  `| \coqdocconstr{C}~\vec{up}
  `| \texttt{?(}\,t\,\texttt{)} `| \texttt{(}x~\textcoloneq~up\texttt{)} \\
  \texttt{user node} & n & \Coloneqq &
                                       \textcoloneq~t~\overrightarrow{where}
                                       `| \texttt{:=!}~\coqdocvar{x} \\
               & & &                                       `|\, \userref{t~\vec{, t}}{clauses}
\end{array}}
...
inside a user node. In the \kw{wf} annotation, the first term should be
...
\ind{WellFounded} on the carrier type. The $\lambda\{$ syntax (using
...
clauses. Local \kw{where} clauses can be used to define recursive
...
scope of \kw{where} and \kw{with} clauses.
...
of abstraction of the enclosing program (introduced by \kw{with} or
not).
