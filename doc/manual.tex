...
\def\figdefs{\begin{array}{llcl}
  ...
  \coqdocvar{x} `| \lambda \coqdocvar{x} : \tau, t `| ∀ \coqdocvar{x} :
                                                    \tau, \tau' `|
  \lambda\{\vecplus{\vec{up} \coloneqq t}\}
   \ldots \\
  ...
  \texttt{program} & p, prog & \Coloneqq & \coqdoccst{f}~Γ~:~τ~(\coqdockw{by}~\textit{annot})?~\coloneqq~clauses \\
  \texttt{annotation} & annot & \Coloneqq & \kw{struct}~x `| \kw{wf}~t~R \\
  \texttt{clauses} & clauses & \Coloneqq & \vec{c} `| \{ \vec{c} \} \\
  \texttt{user clause} & c & \Coloneqq & \coqdoccst{f}~\vec{up}~n `|
                                         \vecplus{\texttt{|} up}~n\\
  \texttt{user pattern} & up & \Coloneqq & x
  `| \coqdocconstr{C}~\vec{up}
  `| \texttt{?}(t) `| (x := up) \\
  \texttt{user node} & n & \Coloneqq & \userref{\vecplus{t}}{clauses}
 `|\,\coloneqq~t~\overrightarrow{where}
\end{array}}
